...
async function handler(m, { conn, args, usedPrefix, command }) {
  if (confirmation[m.sender]) return m.reply('أنت تقوم بإجراء تحويل')
  let user = global.db.data.users[m.sender]
  const item = items.filter(v => v in user && typeof user[v] == 'number')
  let lol = `✳️ الاستخدام الصحيح للأمر 
*${usedPrefix + command}*  credit [المبلغ] [@user]
...
`.trim()
  ...
  if (!(who in global.db.data.users)) return m.reply(`✳️ المستخدم غير موجود في قاعدتي البيانات`)
...
`.trim()
  ...
  confirmation[m.sender] = {
    ...
    count,
    timeout: setTimeout(
      () => (m.reply('⏳ الوقت انتهى'), delete confirmation[m.sender]),
      60 * 1000
    ),
  }
}
...
handler.help = ['give'].map(v => v + ' credit [المبلغ] [@tag]')
