...
let handler = async (m, { conn, args, usedPrefix, command }) => {
  ...
  try {
    ...
    if (/webp/g.test(mime)) {
      ...
      stiker = await addExif(img, name || global.name, '')
    } else if (/image/g.test(mime)) {
      let img = await q.download?.()
      stiker = await createSticker(img, false, name || global.name, '')
    } else if (/video/g.test(mime)) {
      let img = await q.download?.()
      stiker = await mp4ToWebp(img, { pack: name || global.name, author: '' })
    } else if (args[0] && isUrl(args[0])) {
      stiker = await createSticker(false, args[0], '', '', 20)
    } else {
      throw `*RESPOND TO AN IMAGE OR VIDEO OR GIF ${usedPrefix + command}*`
    }
  } catch {
    stiker = '*huh*'
  } finally {
